...
spark.sql("CREATE DATABASE IF NOT EXISTS {0}".format(user_db))
spark.sql("use {0}".format(user_db))
...
spark.sql("cache table {0}".format(batch_temp_view))
...
columns_to_dropped = ["submitted_at", "order_id", "customer_id", "shipping_address_attention", "shipping_address_address", "shipping_address_city", "shipping_address_state", "shipping_address_zip", "product_id", "product_quantity", "product_sold_price"]

columns_to_be_deduped = ["sales_rep_id" ,"sales_rep_ssn" ,"sales_rep_first_name" ,"sales_rep_last_name" ,"sales_rep_address" ,"sales_rep_city" ,"sales_rep_state" ,"sales_rep_zip" ,"_error_ssn_format"]

df = sqlContext.table(batch_temp_view)
df= df.withColumn("_error_ssn_format",when(col("sales_rep_ssn").contains("-"),True).otherwise(False))
df= df.withColumn('sales_rep_ssn',regexp_replace(col("sales_rep_ssn"), '-', ''))
df = df.withColumn("sales_rep_ssn",df["sales_rep_ssn"].cast(LongType())).withColumn("sales_rep_zip",df["sales_rep_zip"].cast(IntegerType()))
df = df.drop(*columns_to_dropped)
df = df.dropDuplicates(columns_to_be_deduped)
df.write.format("delta").saveAsTable(sales_reps_table)
...
columns_to_dropped_02 = ["sales_rep_ssn", "sales_rep_first_name", "sales_rep_last_name", "sales_rep_address", "sales_rep_city", "sales_rep_state", "sales_rep_zip", "product_id", "product_quantity", "product_sold_price"]

columns_to_be_deduped_02 = ["submitted_at" ,"order_id" ,"customer_id" ,"sales_rep_id" ,"shipping_address_attention" ,"shipping_address_address" ,"shipping_address_city" ,"shipping_address_state" ,"shipping_address_zip" ,"ingest_file_name" ,"ingested_at"]

df = sqlContext.table(batch_temp_view)
df=df.withColumn("submitted_at",from_unixtime('submitted_at').cast(TimestampType()))\
   .withColumn("shipping_address_zip",df["shipping_address_zip"].cast(IntegerType()))\
   .drop(*columns_to_dropped_02)\
   .dropDuplicates(columns_to_be_deduped_02)\
   .withColumn("submitted_yyyy_mm", date_format(col('submitted_at'),'yyyy-MM'))
df.write.format("delta").partitionBy("submitted_yyyy_mm").saveAsTable(orders_table)
...
# Use this cell to complete your solution
df = sqlContext.table(batch_temp_view)
df = df.select(col("order_id"),col("product_id"),col("product_quantity"),col("product_sold_price"),col("ingest_file_name"),col("ingested_at"))
df=df.withColumn("product_quantity",df["product_quantity"].cast(IntegerType())).withColumn("product_sold_price",df["product_sold_price"].cast(DecimalType(10,2)))       
df.write.format("delta").saveAsTable(line_items_table)
