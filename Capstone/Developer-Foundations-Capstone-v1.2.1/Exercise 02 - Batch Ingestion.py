...
df = df.select("value",*[f.substring("value",*v).alias(k) 
                     for k,v in fixed_width_column_defs.items()]).drop("value")
df= df.select([trim(col(c)).alias(c) for c in df.columns]).replace('',None).withColumn('ingest_file_name',input_file_name()).withColumn('ingested_at',current_timestamp())
df.write.format("delta").save(batch_target_path)
...
# TODO
# Use this cell to complete your solution
df_2018 = spark.read.option("sep","\t").option("header","true").csv(batch_2018_path)
df_2018 = df_2018.withColumn("ingest_file_name",input_file_name()).withColumn("ingested_at",current_timestamp()).replace('null',None)
...
df_2019 = spark.read.option("header","True").csv(batch_2019_path)
df_2019 = df_2019.withColumn("ingest_file_name",input_file_name()).withColumn("ingested_at",current_timestamp()).replace('null',None).toDF(*df_2018.columns)
