...
def reality_check_06_b():
  ...
  suite.test(f"{suite_name}.table-exists", f"""The table "{question_1_results_table}" exists""",  
             testFunction = lambda: len(list(__builtin__.filter(lambda t: t.name==question_1_results_table, spark.catalog.listTables(user_db)))) == 1)
  
  suite.test(f"{suite_name}.is-temp-view", f"""The table "{question_1_results_table}" is a temp view""", dependsOn=[suite.lastTestId()],
             testFunction = lambda: list(__builtin__.filter(lambda t: t.name==question_1_results_table, spark.catalog.listTables(user_db)))[0].tableType == "TEMPORARY")
...
def reality_check_06_c(ex_avg, ex_min, ex_max):
  ...
  suite.test(f"{suite_name}.table-exists", f"""The table "{question_2_results_table}" exists""",  
             testFunction = lambda: len(list(__builtin__.filter(lambda t: t.name==question_2_results_table, spark.catalog.listTables(user_db)))) == 1)
  
  suite.test(f"{suite_name}.is-temp-view", f"""The table "{question_2_results_table}" is a temp view""", dependsOn=[suite.lastTestId()],
             testFunction = lambda: list(__builtin__.filter(lambda t: t.name==question_2_results_table, spark.catalog.listTables(user_db)))[0].tableType == "TEMPORARY")
...
def reality_check_06_d():
  ...
  suite.test(f"{suite_name}.table-exists", f"""The table "{question_3_results_table}" exists""",  
             testFunction = lambda: len(list(__builtin__.filter(lambda t: t.name==question_3_results_table, spark.catalog.listTables(user_db)))) == 1)
  
  suite.test(f"{suite_name}.is-temp-view", f"""The table "{question_3_results_table}" is a temp view""", dependsOn=[suite.lastTestId()],
             testFunction = lambda: list(__builtin__.filter(lambda t: t.name==question_3_results_table, spark.catalog.listTables(user_db)))[0].tableType == "TEMPORARY")
