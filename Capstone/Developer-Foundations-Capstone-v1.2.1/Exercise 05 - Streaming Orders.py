    ...
    StructField(
        'products', ArrayType(
            StructType([
                ...
                StructField('soldPrice', DecimalType(10,2), True)])),True),
...
df=spark.readStream.option("maxFilesperTrigger",1).option("multiline",True).schema(newschema).json(stream_path)

df = df.withColumn('ingest_file_name',input_file_name()).withColumn('ingested_at',current_timestamp())\
...
df=df.select("customer_id","order_id","sales_rep_id","shippingAddress.*","submitted_at","ingest_file_name","ingested_at","submitted_yyyy_mm")
df=df.select("submitted_at","order_id","customer_id","sales_rep_id","attention","address","city","state","zip","ingest_file_name",
             "ingested_at","submitted_yyyy_mm").withColumn("zip",col("zip").cast(IntegerType()))

df=df.toDF("submitted_at","order_id","customer_id","sales_rep_id","shipping_address_attention","shipping_address_address","shipping_address_city","shipping_address_state","shipping_address_zip","ingest_file_name","ingested_at","submitted_yyyy_mm")

df.writeStream.outputMode("append").format("delta").queryName(orders_table).trigger(processingTime="1 second").option("checkpointLocation", orders_checkpoint_path).table(orders_table)
...
df = spark.readStream.option("maxFilesPerTrigger",1).option("multiline","true").schema(new_schema).json(stream_path)
df = df.withColumn("products",explode(col("products")))\
       .withColumn('ingest_file_name',input_file_name())\
       .withColumn('ingested_at',current_timestamp())

df =df.select(col("orderID"),col("products.*"),col("ingest_file_name"),col("ingested_at"))

df = df.toDF("order_id","product_id","product_quantity","product_sold_price","ingest_file_name","ingested_at")

df.writeStream.outputMode("append").format("delta").queryName(line_items_table).trigger(processingTime="1 second").option("checkpointLocation", line_items_checkpoint_path).table(line_items_table)
