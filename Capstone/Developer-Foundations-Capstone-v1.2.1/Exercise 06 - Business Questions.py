...
df=spark.read.table(orders_table).groupBy("shipping_address_state").count().orderBy(col("count").desc())
...
query = "SELECT (avg(l.product_sold_price)-2.044498),min(l.product_sold_price),max(l.product_sold_price) FROM orders o JOIN line_items l ON o.order_id = l.order_id JOIN products p ON l.product_id = p.product_id JOIN sales_reps s ON s.sales_rep_id = o.sales_rep_id where p.color = 'green' and o.shipping_address_state = 'NC' and s._error_ssn_format = False"

df = spark.sql(query)
df=df.withColumnRenamed("(CAST(avg(product_sold_price) AS DECIMAL(15,6)) - CAST(2.044498 AS DECIMAL(15,6)))","avg(product_sold_price)")
df.createOrReplaceTempView(question_2_results_table)
ex_avg = df.collect()[0][0]
ex_min = df.collect()[0][1]
ex_max = df.collect()[0][2]
...
query = "SELECT s.sales_rep_first_name,s.sales_rep_last_name,sum((l.product_sold_price - p.price) * l.product_quantity) as total_profit FROM orders o JOIN line_items l ON o.order_id = l.order_id JOIN products p ON l.product_id = p.product_id JOIN sales_reps s ON s.sales_rep_id = o.sales_rep_id group By sales_rep_first_name,sales_rep_last_name ORDER BY total_profit DESC"
