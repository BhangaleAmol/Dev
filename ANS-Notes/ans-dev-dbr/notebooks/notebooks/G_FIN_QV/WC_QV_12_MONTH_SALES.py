...
# MAGIC %run ../_SCHEMA/g_fin_qv.wc_qv_12_month_sales
...
table_name = get_input_param('table_name', default_value = 'wc_qv_12_month_sales')
...
main = spark.sql("""
select
  row_number() over (order by (select NULL)) ROW_NUMBER,
  date_format(monthenddate,'MM/01/yyyy') INVOICE_MONTH,
  ...
  --sales_invoice_headers.exchangeRateusd,
  --product.lowestShippableUOM,
  SUM(
  ...
  round(
    ...
    2
  )  RETURN_AMT_USD --sales_invoice_lines.discountLineFlag,
...
where
  ...
  and account.customerType = 'External' --and sales_invoice_headers.invoiceNumber = '134883'
  --and sales_invoice_lines.invoicedetailid = '48584794'
  ...
  and NVL(sales_invoice_lines.quantityInvoiced, 0) <> 0 --and date_format(add_months(monthenddate, -1) + 1, 'MM/dd/yyyy') = '01/01/2021' -- and account.accountNumber = '10003'
  --and product.productcode = '100011'
  -- and invoicenumber = '100015297'
GROUP BY
  account.name,
  account.accountNumber,
  product.productcode,
  product.name,
  date_format(monthenddate,'MM/01/yyyy')
