...
file_name = file_name or "Gross to Net Accruals.xlsx"
sheet_name = sheet_name or 'Gross to Net Basis'
table_name = table_name or "gross_to_net_accruals"
...
main_pd = pd.read_excel(source_file_path, header = header, sheet_name = sheet_name, usecols="A:P", engine='openpyxl')
...
main2 = (
  ...
  .transform(convert_null_to_zero(['SettlementDiscount','VolumeDiscount','EndCustomerRebate','BOGO','Coop','Others','TPR']))
  .transform(convert_null_to_unknown(columns = 'Entity,CustomerID,Region,SubRegion,DIVISION,OPERATINGUNIT,CustomerName,Style'))
  .transform(trim_all_values)
  .distinct()
  .filter("SettlementDiscount<>0.0 or VolumeDiscount<>0.0 or EndCustomerRebate<>0.0 or BOGO<>0.0 or Coop<>0.0 or Others <>0.0 or TPR<>0.0")
)
...
show_duplicate_rows(main_f, ['Entity','CustomerID','Region','SubRegion','DIVISION','OPERATINGUNIT','CustomerName','Style'])

# COMMAND ----------

key_columns = 'Entity,CustomerID,Region,SubRegion,DIVISION,OPERATINGUNIT,CustomerName,Style'
...
if key_columns is not None:
  ...
  if not duplicates.rdd.isEmpty():
    notebook_data = {
      ...
      'duplicates_sample': duplicates.select(['Entity','CustomerID','Region','SubRegion','DIVISION','OPERATINGUNIT','CustomerName','Style']).limit(50)
    }
...
# DROP DUPLICATES
if key_columns is not None:
  main_f = main_f.dropDuplicates(['Entity','CustomerID','Region','SubRegion','DIVISION','OPERATINGUNIT','CustomerName','Style'])
else:
  main_f = main_f
