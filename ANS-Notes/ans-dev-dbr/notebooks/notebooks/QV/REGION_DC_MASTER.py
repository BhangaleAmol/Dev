...
file_name = file_name or "Region_DC_Master.xlsx"
sheet_name = sheet_name or 'Sheet1'
table_name = table_name or "region_dc_master"
...
main_pd = pd.read_excel(source_file_path, header = header, sheet_name = sheet_name, usecols = "A:F", engine='openpyxl',na_filter=False)
...
show_duplicate_rows(main_f, ['Globallocationcode','ERPWHCode'])

# COMMAND ----------

key_columns = 'Globallocationcode,ERPWHCode'
...
if key_columns is not None:
  ...
  if not duplicates.rdd.isEmpty():
    notebook_data = {
      ...
      'duplicates_sample': duplicates.select(['Globallocationcode','ERPWHCode']).limit(50)
    }
...
# DROP DUPLICATES
if key_columns is not None:
  main_f = main_f.dropDuplicates(['Globallocationcode','ERPWHCode'])
else:
  main_f = main_f
