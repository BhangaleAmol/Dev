...
file_name = file_name or "StandardLT.xls"
sheet_name = sheet_name or 'Sheet4'
table_name = table_name or "standardlt"
...
main_pd = pd.read_excel(source_file_path, header = header, sheet_name = sheet_name, usecols = "A:F", engine='xlrd',na_filter=False)
...
show_duplicate_rows(main_f, ['productmainGroup','productSubGroup'])

# COMMAND ----------

key_columns = 'productmainGroup,productSubGroup'
...
if key_columns is not None:
  ...
  if not duplicates.rdd.isEmpty():
    notebook_data = {
      ...
      'duplicates_sample': duplicates.select(['productmainGroup','productSubGroup']).limit(50)
    }
...
# DROP DUPLICATES
if key_columns is not None:
  main_f = main_f.dropDuplicates(['productmainGroup','productSubGroup'])
else:
  main_f = main_f
