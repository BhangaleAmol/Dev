...
# LOAD DATASETS
vw_company = load_full_dataset('col.vw_company')
vw_warehouse = load_full_dataset('col.vw_warehouse')
...
select distinct
   ...
   '' AS commonName,
   'DC819' AS commonOrganizationCode,
   CAST(NULL AS STRING) AS country,
   CAST(NULL AS STRING) AS countryCode,
   'COP' AS currency,
   TRUE AS isActive,
   'Columbia warehouse' AS name,
   '819' AS organizationCode,
   '819' organizationId,
...
SELECT DISTINCT
	...
	COMPANY.Company organizationCode,
...
# HANDLE DELETE
full_company_keys = spark.sql("""SELECT DISTINCT
	COMPANY.Company organizationId
FROM vw_company COMPANY
""")
...
if not test_run:
  ...
  update_cutoff_value(cutoff_value, table_name, 'col.vw_company')
  update_run_datetime(run_datetime, table_name, 'col.vw_company')

  cutoff_value = get_incr_col_max_value(vw_warehouse)
  update_cutoff_value(cutoff_value, table_name, 'col.vw_warehouse')
  update_run_datetime(run_datetime, table_name, 'col.vw_warehouse')
