...
main = spark.sql("""
  SELECT distinct
    ...
    REPLACE(
    STRING(INT (HCA.cust_account_id)),
    ",",
    ""
    ) as accountId,
  ...
  FROM main_inc cc
  LEFT JOIN EBS.HZ_CUST_ACCOUNTS HCA ON cc.CustomerNumber = HCA.account_number 
  Where Company in('2010','5100') and HCA.cust_account_id is not null
  AND TM <> ''
""")
...
full_keys_f = (
  spark.sql("""
    SELECT DISTINCT
      ...
      TM as territoryManager
  FROM tot.tb_pbi_rsm
  LEFT JOIN EBS.HZ_CUST_ACCOUNTS HCA ON tot.tb_pbi_rsm.CustomerNumber = HCA.account_number 
  Where Company in('2010','5100') and HCA.cust_account_id is not null
  AND TM <> ''
  """)
