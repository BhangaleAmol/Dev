...
# UNION
asrs_df2 = asrs_df.select('SITE_ID', 'SITE_SYSTEM_ID', 'SITE_NAME', 'CSTTS')
csrs_df2 = csrs_df.select('SITE_ID', 'SITE_SYSTEM_ID', 'SITE_NAME', 'CSTTS')
...
main_df3 = spark.sql("""
  ...
  FROM (
    SELECT DISTINCT 
      ...
      SITE_NAME,
      ROW_NUMBER() OVER (PARTITION BY SITE_SYSTEM_ID ORDER BY CSTTS DESC) AS NUM
    FROM main_df2    
  ) 
  WHERE NUM = 1
""")
...
main_f = (
  ...
  .transform(attach_surrogate_key(['SITE_SYSTEM_ID_NK','_SOURCE'], name = 'SITE_ID'))
