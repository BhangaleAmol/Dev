...
# UNION
asrs_df2 = asrs_df.select('TEAM_ID', 'TEAM_SYSTEM_ID', 'TEAM_NAME', 'CSTTS')
csrs_df2 = csrs_df.select('TEAM_ID', 'TEAM_SYSTEM_ID', 'TEAM_NAME', 'CSTTS')
...
main_df3 = spark.sql("""
  ...
  FROM (
    SELECT DISTINCT 
      ...
      TEAM_NAME,
      ROW_NUMBER() OVER (PARTITION BY TEAM_SYSTEM_ID ORDER BY CSTTS DESC) AS NUM
    FROM main_df2 
  ) 
  WHERE NUM = 1
""")
...
main_f = (
  ...
  .transform(attach_surrogate_key(['TEAM_SYSTEM_ID_NK','_SOURCE'], name = 'TEAM_ID'))
