...
# MAGIC %run ../_SCHEMA/g_na_tm.wc_qv_pos_manadj_a
...
table_name = get_input_param('table_name', default_value = 'wc_qv_pos_manadj_a')
...
account_ebs = spark.sql("""
select * from s_core.account_agg where _source = 'EBS' and not _deleted
""")
account_ebs.createOrReplaceTempView("account_ebs")
...
select
  ...
  point_of_sales_agg.distributorPartyId DISTRIBUTORID,
  case
    when point_of_sales_agg.distributorPartyId like '% - OTD' then party_agg.partyName
    when point_of_sales_agg.distributorPartyId like '% - ITD' then account.name
  end DISTRIBUTORNAME,
  ...
  COALESCE(
    product_agg.productMaterialSubType,
    'Unspecified'
  ) MATERIALSUBTYPE,
  ...
  nvl(
    ...
    point_of_sales_agg.distributorSubmittedItemNumber
  ) SKU,
  ...
  case when point_of_sales_agg.distributorPartyId like '% - ITD' then account._ID 
       when point_of_sales_agg.distributorPartyId like '% - OTD'  then account_otd._ID end ACCOUNT_ID
from
  s_trademanagement.point_of_sales_agg
  left join s_core.organization_agg org on point_of_sales_agg.owningbusinessunit_id = org._ID
  left join account_ebs account on case
    when point_of_sales_agg.distributorPartyId like '% - ITD' then rtrim(' - ITD', point_of_sales_agg.distributorPartyId)
  end = account.accountnumber and account._source = 'EBS'
  left join s_core.party_agg on case
    when point_of_sales_agg.distributorPartyId like '% - OTD' then rtrim(' - OTD', point_of_sales_agg.distributorPartyId)
  end = party_agg.partyNumber 
  left join s_core.product_ebs product_agg on point_of_sales_agg.item_ID = product_agg._ID --join exclusion_list_otd on party_agg.partyNumber = exclusion_list_otd.RegistrationID
  left join account_ebs account_otd on  case
    when point_of_sales_agg.distributorPartyId like '% - OTD' then rtrim(' - OTD', point_of_sales_agg.distributorPartyId)
  end = account_otd.registrationid and account_otd.accountnumber not in ('11936') and account_otd._source = 'EBS'
where
  point_of_sales_agg._source = 'EBS'
  and not point_of_sales_agg._deleted
  --and not account._deleted
  ...
  --and case
    ---when point_of_sales_agg.distributorPartyId like '% - OTD' then rtrim(' - OTD', point_of_sales_agg.distributorPartyId)
  --end = '6743'
  --and point_of_sales_agg.distributorPartyId = '6743 - OTD'
GROUP BY
  ...
  point_of_sales_agg.distributorPartyId,
  case
    when point_of_sales_agg.distributorPartyId like '% - OTD' then party_agg.partyName
    when point_of_sales_agg.distributorPartyId like '% - ITD' then account.name
  end,
  case when point_of_sales_agg.distributorPartyId like '% - ITD' then account._ID 
       when point_of_sales_agg.distributorPartyId like '% - OTD'  then account_otd._ID end 
""")
