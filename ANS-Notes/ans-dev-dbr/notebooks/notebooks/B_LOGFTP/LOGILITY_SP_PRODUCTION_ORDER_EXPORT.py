...
if 'Prop_0' in main.columns:
  ...
  header = main.filter(main['Prop_0'] == 'LOC_ID')
...
# BASIC CLEANUP
from pyspark.sql.functions import lit,col,current_date,last_day,when,next_day
...
main_f=main_f.select([when(col(c)=="",None).otherwise(col(c)).alias(c) for c in main_f.columns]) # replace Empty Value with None
main_f.cache()
# main_f.display()
