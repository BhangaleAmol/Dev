...
if rowscount > 0:
  ...
  main_2 = main_2.drop("FCSTLVL","Lvl2Fcst3")  

  column_list = main_2.columns
  column_list[0] = 'Lvl1Fcst'
  column_list[1] = 'Lvl2Fcst'
  column_list[2] = 'Description' 
  column_list[3] = 'ForecastPlanner'
...
if rowscount > 0:
  ...
  for i in range(1, 25):
    ...
    for c in main_4.columns:
      if c == f'ORDQ{future_date.month}/{str(future_date.year)[2:]}':
        main_4 = main_4.withColumnRenamed(c, f'ORDQ{i}')
      if c == f'BLDIQ{future_date.month}/{str(future_date.year)[2:]}':
        main_4 = main_4.withColumnRenamed(c, f'BLDIQ{i}')
      if c == f'AVSL{future_date.month}/{str(future_date.year)[2:]}':
        main_4 = main_4.withColumnRenamed(c, f'AVSL{i}')
      if c == f'AVSS{future_date.month}/{str(future_date.year)[2:]}':
        main_4 = main_4.withColumnRenamed(c, f'AVSS{i}')
      if c == f'DPFC{future_date.month}/{str(future_date.year)[2:]}':
        main_4 = main_4.withColumnRenamed(c, f'DPFC{i}')
...
# VALIDATE DATA
if key_columns is not None and rowscount > 0:
