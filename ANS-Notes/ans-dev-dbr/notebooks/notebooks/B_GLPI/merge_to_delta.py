...
if append_only:
  if not sampling:
    register_hive_table(main_f, full_table_name, target_folder, options = {'overwrite': overwrite, 'file_extension': 'par'})
    append_into_table(main_f, full_table_name, options = {'incremental_column': incremental_column})

elif key_columns is None:
  if not sampling:
    register_hive_table(main_f, full_table_name, target_folder, options = {'overwrite': True, 'file_extension': 'par'})
