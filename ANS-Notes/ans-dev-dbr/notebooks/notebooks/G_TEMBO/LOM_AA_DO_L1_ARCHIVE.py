...
table_name = get_input_param('table_name', default_value = 'LOM_AA_DO_L1_ARCHIVE')
...
TMP_LOM_AA_DO_L1 = spark.read.format('delta').load(file_path)
TMP_LOM_AA_DO_L1.createOrReplaceTempView("TMP_LOM_AA_DO_L1")
...
# SAMPLING
if sampling:
  TMP_LOM_AA_DO_L1 = TMP_LOM_AA_DO_L1.limit(10)
  TMP_LOM_AA_DO_L1.createOrReplaceTempView('TMP_LOM_AA_DO_L1')
...
main_df = spark.sql("""
 ...
 where 
   ...
   and User_Data_84 not like '#Error%'
   and User_Data_01 not like '#Error%'
   and User_Data_02 not like '#Error%'
   and User_Data_03 not like '#Error%'
   and User_Data_04 not like '#Error%'
   and User_Data_05 not like '#Error%'
   and User_Data_06 not like '#Error%'
   and User_Data_07 not like '#Error%'
   and nvl(User_Data_08,'Include') not like '#Error%'
   and User_Data_09 not like '#Error%'
   and nvl(User_Data_10,'Include') not like '#Error%'
   and User_Data_11 not like '#Error%'
   and User_Data_12 not like '#Error%'
   and User_Data_13 not like '#Error%'
   and User_Data_14 not like '#Error%'
   and User_Data_15 not like '#Error%'
   and nvl(User_Data_16,'Include') not like '#Error%'
   and User_Data_23 not like '#Error%'
   and User_Data_30 not like '#Error%'
   and User_Data_31 not like '#Error%'
   and User_Data_32 not like '#Error%'
   and User_Data_33 not like '#Error%'
