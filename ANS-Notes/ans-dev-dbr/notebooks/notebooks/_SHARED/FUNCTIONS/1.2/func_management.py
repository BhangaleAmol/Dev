...
def fix_hive_table_partition(table_name):
  try:
    partition = get_hive_table_partition(table_name)    
    if not partition:
      print(table_name)
      part_columns = ['_SOURCE','_PART'] if table_name.endswith("_agg") else ['_PART']
      alter_hive_table_partition(table_name, part_columns)
