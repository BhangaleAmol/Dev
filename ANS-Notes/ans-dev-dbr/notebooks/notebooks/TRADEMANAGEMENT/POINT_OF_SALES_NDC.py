...
ndc = spark.table('amazusftp1.ndc')
...
ndc.createOrReplaceTempView('ndc')
...
SELECT
  ...
  ndc.TRANSACTIONDATE  AS createdOn,
  ...
  CAST(NVL((sum(ndc.QUANTITY * ndc.UNITPRICE)/ SUM(ndc.QUANTITY)),0) AS decimal(22,7))  ansellContractPrice,
  ...
  SUM(ndc.QUANTITY) AS claimedQuantity,
   sum( ((CASE 
WHEN UPPER(ndc.UOM) = 'BD' THEN PROD.piecesInBundle
WHEN UPPER(ndc.UOM) = 'BG' THEN PROD.piecesInBag
WHEN UPPER(ndc.UOM) = 'BX' THEN PROD.piecesInBox
WHEN UPPER(ndc.UOM) = 'CA' THEN 1
WHEN UPPER(ndc.UOM) = 'CS' THEN 1
WHEN UPPER(ndc.UOM) = 'CT' THEN PROD.piecesInCarton
WHEN UPPER(ndc.UOM) = 'DP' THEN PROD.piecesInDisplayDispenser
WHEN UPPER(ndc.UOM) = 'DZ' THEN PROD.piecesInDozen
WHEN UPPER(ndc.UOM) = 'EA' THEN PROD.piecesInEach
WHEN UPPER(ndc.UOM) = 'GR' THEN PROD.piecesInGross
WHEN UPPER(ndc.UOM) = 'PC' THEN 1
WHEN UPPER(ndc.UOM) = 'PK' THEN PROD.piecesInPack
WHEN UPPER(ndc.UOM) = 'PR' THEN 2
WHEN UPPER(ndc.UOM) = 'RL' THEN PROD.piecesInRoll
END ) * ndc.QUANTITY)

/ CASE WHEN UPPER(ndc.UOM) IN ('CA','CS') THEN 1 ELSE PROD.piecesInCase END ) claimedQuantityCase,
  sum( ((CASE 
WHEN UPPER(ndc.UOM) = 'BD' THEN PROD.piecesInBundle
WHEN UPPER(ndc.UOM) = 'BG' THEN PROD.piecesInBag
WHEN UPPER(ndc.UOM) = 'BX' THEN PROD.piecesInBox
WHEN UPPER(ndc.UOM) = 'CA' THEN PROD.piecesInCase
WHEN UPPER(ndc.UOM) = 'CS' THEN PROD.piecesInCase
WHEN UPPER(ndc.UOM) = 'CT' THEN PROD.piecesInCarton
WHEN UPPER(ndc.UOM) = 'DP' THEN PROD.piecesInDisplayDispenser
WHEN UPPER(ndc.UOM) = 'DZ' THEN PROD.piecesInDozen
WHEN UPPER(ndc.UOM) = 'EA' THEN PROD.piecesInEach
...
END ) )AS claimedQuantityPrimary,
  ndc.UOM AS claimedUom,
  NULL AS claimId,
  NULL AS claimNumber,
  NULL AS claimStatus,
  NULL AS contractEligible,
  'USD' AS currency,
  ndc.TRANSACTIONDATE AS dateInvoiced,
  ...
  ndc.END_USER_CITY AS distributorSubmittedEnduserCity,
  ...
  ndc.ENDUSER AS distributorSubmittedEndUserName,
  ndc.STATE_CODE AS distributorSubmittedEndUserState,
  ndc.FIVE_DIGIT_ZIP_CODE AS distributorSubmittedEndUserZipCode,
  ndc.MANUFACTURER_PART_NO AS distributorSubmittedItemNumber,
  ...
  NULL AS gpoStatus,
  CASE WHEN PROD.ITEMID = 'Unknown' THEN ndc.MANUFACTURER_PART_NO ELSE PROD.ITEMID END AS itemId,
  ...
  NULL rejectReason,
  ndc.TRANSACTIONDATE AS reportDate,
  ...
  sum(ndc.QUANTITY * ndc.UNITPRICE) AS transactionAmount,
  ...
  ndc.VERTICAL AS vertical
FROM
 main_stage ndc 
  LEFT JOIN S_CORE.ACCOUNT_AGG ACCOUNT ON ACCOUNT.registrationId = (
    case
      when ndc.DistributorID like '% - OTD' then rtrim(' - OTD', ndc.DistributorID)
      when ndc.DistributorID like '% - ITD' then rtrim(' - ITD', ndc.DistributorID)
      else ndc.DistributorID
    end
  )
  ...
  LEFT JOIN S_CORE.organization_agg ORG ON upper(ORG.name) = upper(ndc.COMPANY)
  LEFT JOIN ZIP3 ON substr(ndc.FIVE_DIGIT_ZIP_CODE, 1, 3) = ZIP3.zip3
  AND ndc.vertical = ZIP3.vertical
  LEFT JOIN ZIP0 ON ndc.vertical = ZIP0.vertical
  LEFT JOIN SECONDORG_ZIP3 ON substr(ndc.FIVE_DIGIT_ZIP_CODE, 1, 3) = SECONDORG_ZIP3.zip3
  AND ndc.vertical = SECONDORG_ZIP3.vertical --LEFT JOIN SECONDORG_ZIP0 ON ndc.vertical                   = SECONDORG_ZIP0.vertical
  LEFT JOIN PROD_CONV ON ndc.MANUFACTURER_PART_NO = PROD_CONV.external_code
  LEFT JOIN smartsheets.qvpos_product_territory_override PROD_OVERRIDE ON PROD_CONV.item = PROD_OVERRIDE.PRODUCTID
  LEFT JOIN CUST_OVERRIDE ON ndc.DistributorID = CUST_OVERRIDE.custid
  ...
  LEFT JOIN SECONDORG_CUST_OVERRIDE ON ndc.DistributorID = SECONDORG_CUST_OVERRIDE.custid
  LEFT JOIN TERR_DEFAULT ON ndc.vertical = TERR_DEFAULT.vertical
  AND SUBSTR(ndc.FIVE_DIGIT_ZIP_CODE, 1, 3) = TERR_DEFAULT.zip3
  LEFT JOIN THIRDORG_PROD_OVERRIDE ON PROD_CONV.item = THIRDORG_PROD_OVERRIDE.PRODUCTID
  LEFT JOIN THIRDORG_CUST_OVERRIDE ON ndc.DistributorID = THIRDORG_CUST_OVERRIDE.custid
  LEFT JOIN THIRDORG_ZIP3 ON substr(ndc.FIVE_DIGIT_ZIP_CODE, 1, 3) = THIRDORG_ZIP3.zip3
...
GROUP BY
  ...
  ndc.ENDUSER,
  ndc.STATE_CODE,
  ndc.FIVE_DIGIT_ZIP_CODE,
  ndc.MANUFACTURER_PART_NO,
  CASE WHEN PROD.ITEMID = 'Unknown' THEN ndc.MANUFACTURER_PART_NO ELSE PROD.ITEMID END,
  ORG.organizationId,
  ZIP3.sales_region,
...
main_f = (
  ...
  .transform(attach_dataset_column('NDC'))
...
full_keys = spark.sql("""
SELECT 
CAST(NVL((sum(ndc.QUANTITY * ndc.UNITPRICE)/ SUM(ndc.QUANTITY)),0) AS decimal(22,7))   AS ansellContractPrice,
NULL AS batchType,
ndc.UOM AS claimedUom,
NULL   AS claimId,
NULL   AS claimNumber,
...
full_keys_f = (
   full_keys
  .transform(parse_date(['dateInvoiced'], expected_format = 'MM/dd/yy'))
  .transform(convert_null_to_unknown(key_columns))
...
filter_date = spark.sql("""select distinct Date_Format(to_Date(TRANSACTIONDATE,'M/d/yyyy'),'yyyy-MM-dd') TransactionDate from amazusftp1.ndc """).rdd.flatMap(lambda x: x).collect()
