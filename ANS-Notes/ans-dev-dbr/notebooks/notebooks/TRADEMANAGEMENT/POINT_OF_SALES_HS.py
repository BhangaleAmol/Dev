...
henrischein = spark.table('amazusftp1.Henri_Schein')
...
if sampling:
  ...
  henrischein = henrischein.limit(10)

# COMMAND ----------

# CREATE VIEWS
terr_df.createOrReplaceTempView("terr_df")
henrischein.createOrReplaceTempView('henrischein')
...
END ) )AS claimedQuantityPrimary,
  ...
  hs.DistributorName2 AS distributorSubmittedEndUserName,
...
main_f = (
  ...
  .transform(attach_dataset_column('HS'))
...
full_keys_f = (
   full_keys
  .transform(parse_date(['dateInvoiced'], expected_format = 'M/dd/yy'))
  .transform(convert_null_to_unknown(key_columns))
...
filter_date = spark.sql("""select distinct Date_Format(to_Date(TransactionDate,'M/d/yyyy'),'yyyy-MM-dd') TransactionDate from amazusftp1.Henri_Schein """).rdd.flatMap(lambda x: x).collect()
