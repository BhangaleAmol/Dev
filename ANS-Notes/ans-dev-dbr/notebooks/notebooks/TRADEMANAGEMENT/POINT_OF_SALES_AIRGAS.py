...
airgas = spark.table('amazusftp1.airgas')
...
if sampling:
  ...
  airgas = airgas.limit(10)

# COMMAND ----------

# CREATE VIEWS
terr_df.createOrReplaceTempView('terr_df')
airgas.createOrReplaceTempView('airgas')
...
SELECT
  ...
  CAST(NVL(airgas.UNITPRICE,0) AS decimal(22,7))  ansellContractPrice,
  ...
  SUM(airgas.UNITS_SOLD) AS claimedQuantity,
   sum( ((CASE 
WHEN UPPER(airgas.UOM_NAME) = 'BD' THEN PROD.piecesInBundle
WHEN UPPER(airgas.UOM_NAME) = 'BG' THEN PROD.piecesInBag
WHEN UPPER(airgas.UOM_NAME) = 'BX' THEN PROD.piecesInBox
WHEN UPPER(airgas.UOM_NAME) = 'CA' THEN PROD.piecesInCase
WHEN UPPER(airgas.UOM_NAME) = 'CT' THEN PROD.piecesInCarton
WHEN UPPER(airgas.UOM_NAME) = 'DP' THEN PROD.piecesInDisplayDispenser
WHEN UPPER(airgas.UOM_NAME) = 'DZ' THEN PROD.piecesInDozen
WHEN UPPER(airgas.UOM_NAME) = 'EA' THEN PROD.piecesInEach
WHEN UPPER(airgas.UOM_NAME) = 'GR' THEN PROD.piecesInGross
WHEN UPPER(airgas.UOM_NAME) = 'PC' THEN 1
WHEN UPPER(airgas.UOM_NAME) = 'PK' THEN PROD.piecesInPack
WHEN UPPER(airgas.UOM_NAME) = 'PR' THEN 2
WHEN UPPER(airgas.UOM_NAME) = 'RL' THEN PROD.piecesInRoll
END ) * airgas.UNITS_SOLD)
...
END ) )AS claimedQuantityPrimary,
 airgas.UOM_NAME AS claimedUom,
  NULL AS claimId,
  NULL AS claimNumber,
  NULL AS claimStatus,
  NULL AS contractEligible,
  'USD' AS currency,
  airgas.BILL_DATE AS dateInvoiced,
  ...
  airgas.CITY AS distributorSubmittedEnduserCity,
  ...
  airgas.SELLING_PLANT_DESCRIPTION AS distributorSubmittedEndUserName,
  airgas.state AS distributorSubmittedEndUserState,
  airgas.SHIP_ZIP_CODE AS distributorSubmittedEndUserZipCode,
  airgas.SUPPLIER_STOCK_NO AS distributorSubmittedItemNumber,
  ...
  NULL AS gpoStatus,
  CASE WHEN PROD.ITEMID = 'Unknown' THEN airgas.SUPPLIER_STOCK_NO ELSE PROD.ITEMID END AS itemId,
  airgas.GIS_BRAND_LEVEL_1 as marketLevel1,
  airgas.GIS_BRAND_LEVEL_2 as marketLevel2,
  airgas.GIS_BRAND_LEVEL_3 as marketLevel3,
  ORG.organizationId AS owningBusinessUnitId,
  'POS' AS posFlag,
   'airgas'   AS posSource,
  'PROCESSED' AS processingType,
  NULL rejectReason,
  airgas.BILL_DATE AS reportDate,
  ...
  sum(airgas.UNITS_SOLD * airgas.UNITPRICE) AS transactionAmount,
  ...
  airgas.VERTICAL AS vertical
FROM
  main_stage airgas
  LEFT JOIN S_CORE.ACCOUNT_AGG ACCOUNT ON ACCOUNT.registrationId = (
    case
      when airgas.DISTID like '% - OTD' then rtrim(' - OTD', airgas.DISTID)
      when airgas.DISTID like '% - ITD' then rtrim(' - ITD', airgas.DISTID)
      else airgas.DISTID
    end
  )
  ...
  LEFT JOIN S_CORE.organization_agg ORG ON upper(ORG.name) = upper(airgas.COMPANY)
  LEFT JOIN ZIP3 ON substr(airgas.SHIP_ZIP_CODE, 1, 3) = ZIP3.zip3
  AND airgas.vertical = ZIP3.vertical
  LEFT JOIN ZIP0 ON airgas.vertical = ZIP0.vertical
  LEFT JOIN SECONDORG_ZIP3 ON substr(airgas.SHIP_ZIP_CODE, 1, 3) = SECONDORG_ZIP3.zip3
  AND airgas.vertical = SECONDORG_ZIP3.vertical --LEFT JOIN SECONDORG_ZIP0 ON airgas.vertical                   = SECONDORG_ZIP0.vertical
  LEFT JOIN PROD_CONV ON airgas.SUPPLIER_STOCK_NO = PROD_CONV.external_code
  LEFT JOIN smartsheets.qvpos_product_territory_override PROD_OVERRIDE ON PROD_CONV.item = PROD_OVERRIDE.PRODUCTID
  LEFT JOIN CUST_OVERRIDE ON airgas.DISTID = CUST_OVERRIDE.custid
  ...
  LEFT JOIN SECONDORG_CUST_OVERRIDE ON airgas.DISTID = SECONDORG_CUST_OVERRIDE.custid
  LEFT JOIN TERR_DEFAULT ON airgas.vertical = TERR_DEFAULT.vertical
  AND SUBSTR(airgas.SHIP_ZIP_CODE, 1, 3) = TERR_DEFAULT.zip3
  LEFT JOIN THIRDORG_PROD_OVERRIDE ON PROD_CONV.item = THIRDORG_PROD_OVERRIDE.PRODUCTID
  LEFT JOIN THIRDORG_CUST_OVERRIDE ON airgas.DISTID = THIRDORG_CUST_OVERRIDE.custid
  LEFT JOIN THIRDORG_ZIP3 ON substr(airgas.SHIP_ZIP_CODE, 1, 3) = THIRDORG_ZIP3.zip3
...
GROUP BY
  ...
  PARTY.partyId,
  airgas.CITY,
  airgas.SELLING_PLANT_DESCRIPTION,
  airgas.state,
  airgas.SHIP_ZIP_CODE,
  airgas.SUPPLIER_STOCK_NO,
  CASE WHEN PROD.ITEMID = 'Unknown' THEN airgas.SUPPLIER_STOCK_NO ELSE PROD.ITEMID END,
  airgas.GIS_BRAND_LEVEL_1 ,
  airgas.GIS_BRAND_LEVEL_2 ,
  airgas.GIS_BRAND_LEVEL_3 ,
  ORG.organizationId,
  ZIP3.sales_region,
...
main_f = (
  ...
  .transform(attach_dataset_column('AIRGAS'))
...
full_keys = spark.sql("""
SELECT 
CAST(NVL(airgas.UNITPRICE,0) AS decimal(22,7))  AS ansellContractPrice,
 NULL AS batchType,
airgas.UOM_NAME AS claimedUom,
NULL   AS claimId,
NULL   AS claimNumber,
...
amazusftp1.airgas airgas
 LEFT JOIN S_CORE.ACCOUNT_AGG ACCOUNT ON ACCOUNT.registrationId = (
  ...
  LEFT JOIN ZIP0 ON airgas.vertical = ZIP0.vertical
  LEFT JOIN SECONDORG_ZIP3 ON substr(airgas.SHIP_ZIP_CODE, 1, 3) = SECONDORG_ZIP3.zip3
  AND airgas.vertical = SECONDORG_ZIP3.vertical 
  LEFT JOIN PROD_CONV ON airgas.SUPPLIER_STOCK_NO = PROD_CONV.external_code
  LEFT JOIN smartsheets.qvpos_product_territory_override PROD_OVERRIDE ON PROD_CONV.item = PROD_OVERRIDE.PRODUCTID
  LEFT JOIN CUST_OVERRIDE ON airgas.DISTID = CUST_OVERRIDE.custid
  LEFT JOIN S_CORE.PRODUCT_EBS PROD ON PROD_CONV.ITEM = PROD.PRODUCTCODE
...
filter_date = spark.sql("""select distinct Date_Format(to_Date(BILL_DATE,'M/d/yyyy'),'yyyy-MM-dd') TransactionDate from amazusftp1.airgas """).rdd.flatMap(lambda x: x).collect()
