...
def get_datasets():
    return {
        'ebs.hz_cust_accounts': read_test_data('/s_core/account_ebs/ebs.hz_cust_accounts.csv'),
        'ebs.hz_parties': read_test_data('/s_core/account_ebs/ebs.hz_parties.csv')
    }
...
@pytest.fixture(scope='module')
def expected_df():
    return read_test_data('/s_core/account_ebs/_expected.csv')


def test_account_ebs(datasets, expected_df, mocker, details):

    mocker.patch(
        'databricks.functions.s_core.account_ebs.add_account_keys',
        lambda df: df)
    mocker.patch(
        'databricks.functions.s_core.account_ebs.update_map_table',
        lambda x, y, z: 0)

    from databricks.functions.s_core.account_ebs import transform_data
