...
def get_datasets():
    return {
        'sf.account': read_test_data('/s_core/account_sf/sf.account.csv'),
        'smartsheets.edm_control_table': read_test_data('/s_core/account_sf/smartsheets.edm_control_table.csv')
    }
...
@pytest.fixture(scope='module')
def expected_df():
    return read_test_data('/s_core/account_sf/_expected.csv')


def test_account_sf(datasets, expected_df, mocker, details):

    mocker.patch(
        'databricks.functions.s_core.account_sf.add_account_keys',
        lambda df: df)
    mocker.patch(
        'databricks.functions.s_core.account_sf.update_map_table',
        lambda x, y, z: 0)

    from databricks.functions.s_core.account_sf import transform_data
