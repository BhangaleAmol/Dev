...
def get_datasets():
    return {
        'tot.tb_pbi_customer': read_test_data('/s_core/account_tot/tot.tb_pbi_customer.csv'),
        'tot.tb_pbi_orders': read_test_data('/s_core/account_tot/tot.tb_pbi_orders.csv')
    }
...
@pytest.fixture(scope='module')
def expected_df():
    return read_test_data('/s_core/account_tot/_expected.csv')


def test_account_tot(datasets, expected_df, mocker, details):

    mocker.patch(
        'databricks.functions.s_core.account_tot.add_account_keys',
        lambda df: df)
    mocker.patch(
        'databricks.functions.s_core.account_tot.update_map_table',
        lambda x, y, z: 0)

    from databricks.functions.s_core.account_tot import transform_data
