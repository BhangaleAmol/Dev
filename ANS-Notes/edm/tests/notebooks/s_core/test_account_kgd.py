...
def get_datasets():
    return {
        'kgd.dbo_tembo_customer': read_test_data('/s_core/account_kgd/kgd.dbo_tembo_customer.csv')
    }
...
@pytest.fixture(scope='module')
def expected_df():
    return read_test_data('/s_core/account_kgd/_expected.csv')


def test_account_kgd(datasets, expected_df, mocker, details):

    mocker.patch(
        'databricks.functions.s_core.account_kgd.add_account_keys',
        lambda df: df)
    mocker.patch(
        'databricks.functions.s_core.account_kgd.update_map_table',
        lambda x, y, z: 0)
    ...
    table_schema = get_dict_schema_from_config('s_core.account_kgd')
    excepted_df = (
        expected_df
