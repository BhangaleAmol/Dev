...
def get_datasets():
    return {
        'sapp01.kna1': read_test_data('/s_core/account_sap/sapp01.kna1.csv')
    }
...
@pytest.fixture(scope='module')
def expected_df():
    return read_test_data('/s_core/account_sap/_expected.csv')


def test_account_sap(datasets, expected_df, mocker, details):

    mocker.patch(
        'databricks.functions.s_core.account_sap.add_account_keys',
        lambda df: df)
    mocker.patch(
        'databricks.functions.s_core.account_sap.update_map_table',
        lambda x, y, z: 0)

    from databricks.functions.s_core.account_sap import transform_data
